...
extern volatile uint8 USBUART_1_device;
    ...
    ******************************************************************************/
    ...
    {
        #if defined(USBUART_1_ENABLE_MIDI_STREAMING) && (USBUART_1_ENABLE_MIDI_API != 0u) && \
            (USBUART_1_EP_MM == USBUART_1__EP_DMAAUTO) && (CY_PSOC3)
            uint8 int_en;
        ...
        CY_SET_REG8(USBUART_1_SIE_EP_INT_SR_PTR, CY_GET_REG8(USBUART_1_SIE_EP_INT_SR_PTR)
                                                                            & ~USBUART_1_SIE_EP_INT_EP1_MASK);

        #if defined(USBUART_1_ENABLE_MIDI_STREAMING) && (USBUART_1_ENABLE_MIDI_API != 0u) && \
            (USBUART_1_EP_MM == USBUART_1__EP_DMAAUTO) 
    ...
    *******************************************************************************/
    CY_ISR(USBUART_1_ARB_ISR)
    {
        ...
        while(int_status != 0u)
        {
            ...
            {
                ep_status  = USBUART_1_ARB_EP1_SR_PTR[ptr];       /* read Endpoint Status Register */
                /* If In Buffer Full */
                if(ep_status & USBUART_1_ARB_EPX_SR_IN_BUF_FULL)
                {
                ...
                /* (re)arm Out EP only for mode2 */
                    ...
                    /* If DMA Grant */
                    if(ep_status & USBUART_1_ARB_EPX_SR_DMA_GNT)
                    {
                ...
                USBUART_1_ARB_EP1_SR_PTR[ptr] = ep_status;       /* Clear Serviced events */
            }
