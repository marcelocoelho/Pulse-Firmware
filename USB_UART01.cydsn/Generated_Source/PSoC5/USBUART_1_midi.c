...
const uint8 CYCODE USBUART_1_MIDI_SIZE[] = {
...
volatile uint8 USBUART_1_midiInBuffer[USBUART_1_MIDI_IN_BUFF_SIZE];   /* Input endpoint buffer */
volatile uint8 USBUART_1_midiOutBuffer[USBUART_1_MIDI_OUT_BUFF_SIZE]; /* Output endpoint buffer */
...
#if (USBUART_1_MIDI_EXT_MODE >= USBUART_1_ONE_EXT_INTRF)
    volatile USBUART_1_MIDI_RX_STATUS USBUART_1_MIDI1_Event;          /* MIDI RX status structure */
    volatile uint8 USBUART_1_MIDI1_TxRunStat;                                /* MIDI Output running status */
    volatile uint8 USBUART_1_MIDI1_InqFlags;                                 /* Device inquiry flag */    
    
#if (USBUART_1_MIDI_EXT_MODE >= USBUART_1_TWO_EXT_INTRF)
    volatile USBUART_1_MIDI_RX_STATUS USBUART_1_MIDI2_Event;          /* MIDI RX status structure */
    volatile uint8 USBUART_1_MIDI2_TxRunStat;                                /* MIDI Output running status */
...
void USBUART_1_PrepareInBuffer(uint8 ic, uint8 *srcBuff, uint8 eventLen, uint8 cable)
                                                                 ;
#if (USBUART_1_MIDI_EXT_MODE >= USBUART_1_ONE_EXT_INTRF)
    void USBUART_1_MIDI_Init(void) ;

    uint8 USBUART_1_MIDI1_GetEvent(void) ;
    void USBUART_1_MIDI1_ProcessUsbOut(uint8 *epBuf) ;
    
    #if (USBUART_1_MIDI_EXT_MODE >= USBUART_1_TWO_EXT_INTRF)
        uint8 USBUART_1_MIDI2_GetEvent(void) ;
        void USBUART_1_MIDI2_ProcessUsbOut(uint8 *epBuf) 
                                                    ;
    #endif /* End USBUART_1_MIDI_EXT_MODE >= USBUART_1_TWO_EXT_INTRF */
#endif /* End USBUART_1_MIDI_EXT_MODE >= USBUART_1_ONE_EXT_INTRF */
...
void USBUART_1_MIDI_EP_Init(void) 
{
    USBUART_1_midiInPointer = 0u;
        ...
        USBUART_1_LoadInEP(USBUART_1_midi_in_ep, (uint8 *)USBUART_1_midiInBuffer,
                                                                                USBUART_1_MIDI_IN_BUFF_SIZE);
        /* Init DMA configurations for OUT EP*/
        USBUART_1_ReadOutEP(USBUART_1_midi_out_ep, (uint8 *)USBUART_1_midiOutBuffer, 
    ...
    if (USBUART_1_GetEPState(USBUART_1_midi_out_ep) == USBUART_1_OUT_BUFFER_FULL)
    {
        outLength = USBUART_1_GetEPCount(USBUART_1_midi_out_ep);
            ...
            outLength = USBUART_1_ReadOutEP(USBUART_1_midi_out_ep, 
                                                   (uint8 *)USBUART_1_midiOutBuffer, outLength);
            #if(USBUART_1_EP_MM == USBUART_1__EP_DMAMANUAL)
                do  /* wait for DMA transfer complete */
                {
                    CyDmaChStatus(USBUART_1_DmaChan[USBUART_1_midi_out_ep], NULL, &dmaState);
        ...
        if (outLength >= USBUART_1_EVENT_LENGTH)
        {
            ...
            while (outPointer < outLength)
            {
                ...
                /* Route USB MIDI to the External connection */
                    ...
                    {
                        USBUART_1_MIDI1_ProcessUsbOut((uint8 *)&USBUART_1_midiOutBuffer[outPointer]);
                    }
                    else if ((USBUART_1_midiOutBuffer[outPointer] & USBUART_1_CABLE_MASK) == 
                             USBUART_1_MIDI_CABLE_01)
                    {
                        #if (USBUART_1_MIDI_EXT_MODE >= USBUART_1_TWO_EXT_INTRF)
                            USBUART_1_MIDI2_ProcessUsbOut((uint8 *)&USBUART_1_midiOutBuffer[outPointer]);
                        #endif /* End USBUART_1_MIDI_EXT_MODE >= USBUART_1_TWO_EXT_INTRF */
                    }
                    else
                    {
                ...
                USBUART_1_callbackLocalMidiEvent(
                    ...
                    (uint8 *)&USBUART_1_midiOutBuffer[outPointer + USBUART_1_EVENT_BYTE1]);
...
void USBUART_1_MIDI_IN_EP_Service(void) 
{
    ...
    if ((USBUART_1_midiInPointer != 0u) && 
        (USBUART_1_GetEPState(USBUART_1_midi_in_ep) == USBUART_1_EVENT_PENDING))
    {
        #if(USBUART_1_EP_MM != USBUART_1__EP_DMAAUTO)
            USBUART_1_LoadInEP(USBUART_1_midi_in_ep, (uint8 *)USBUART_1_midiInBuffer,
            ...
            /* rearm IN EP */
            USBUART_1_LoadInEP(USBUART_1_midi_in_ep, NULL, USBUART_1_midiInPointer);
        #endif /* End USBUART_1_EP_MM != USBUART_1__EP_DMAAUTO*/
    ...
    */
        ...
        do 
        {
            if (USBUART_1_midiInPointer <= (USBUART_1_MIDI_IN_BUFF_SIZE - USBUART_1_EVENT_LENGTH))
            {
                ...
                if ((m1 = USBUART_1_MIDI1_GetEvent()) != 0u)
                {
            ...
            if (USBUART_1_midiInPointer <= (USBUART_1_MIDI_IN_BUFF_SIZE - USBUART_1_EVENT_LENGTH))
            {
                ...
                if ((m2 = USBUART_1_MIDI2_GetEvent()) != 0u)
                {
             ...
             (USBUART_1_midiInPointer <= (USBUART_1_MIDI_IN_BUFF_SIZE - USBUART_1_EVENT_LENGTH)));
    #endif /* End USBUART_1_MIDI_EXT_MODE >= USBUART_1_ONE_EXT_INTRF */
...
*
*******************************************************************************/
uint8 USBUART_1_PutUsbMidiIn(uint8 ic, uint8* midiMsg, uint8 cable)
                                                            
{
                ...
                (USBUART_1_EP[USBUART_1_midi_in_ep].bufferSize - USBUART_1_EVENT_LENGTH))
    {
        ...
        {   /* only SysEx message is greater than 4 bytes */
            do
            {
                USBUART_1_PrepareInBuffer(USBUART_1_MIDI_SYSEX, midiMsg, 
                                                 USBUART_1_EVENT_BYTE3, cable);
                ic -= USBUART_1_EVENT_BYTE3;
                midiMsg += USBUART_1_EVENT_BYTE3;
                if (USBUART_1_midiInPointer > 
                    (USBUART_1_EP[USBUART_1_midi_in_ep].bufferSize - USBUART_1_EVENT_LENGTH))
                {
                    USBUART_1_MIDI_IN_EP_Service();
    ...
    else if((ic < USBUART_1_EVENT_LENGTH) || (ic == USBUART_1_MIDI_SYSEX))
    {
        ...
        else if (srcBuffZero < USBUART_1_MIDI_SYSEX)
        {
            USBUART_1_midiInBuffer[USBUART_1_midiInPointer] =
                                                    ((srcBuffZero >> 4u) & USBUART_1_CIN_MASK) | cable;
            USBUART_1_midiInPointer++;
        }
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBUART_1_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            ...
            {
                if ((mData & USBUART_1_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(singl byte) */
                {
                    midiReturn = mData;
                }
