...
{
    if (Clock_2_CLKEN & Clock_2_CLKEN_MASK)
    {
...
*
*******************************************************************************/
void Clock_2_SetModeRegister(uint8 clkMode) 
{
    Clock_2_MOD_SRC |= clkMode & Clock_2_MODE_MASK;
}
...
*
*******************************************************************************/
void Clock_2_ClearModeRegister(uint8 clkMode) 
{
    Clock_2_MOD_SRC &= ~clkMode | ~Clock_2_MODE_MASK;
}
...
{
    return Clock_2_MOD_SRC & Clock_2_MODE_MASK;
}
...
void Clock_2_SetSourceRegister(uint8 clkSource) 
{
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clock_2_MOD_SRC |= CYCLK_SSS;
        Clock_2_MOD_SRC =
            (Clock_2_MOD_SRC & ~Clock_2_SRC_SEL_MSK) | clkSource;
    }
