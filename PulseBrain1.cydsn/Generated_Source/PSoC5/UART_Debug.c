...
void UART_Debug_Init(void) 
{
        ...
        /* Write Counter Value for TX Bit Clk Generator*/
            ...
            UART_Debug_TXBITCLKTX_COMPLETE_REG = (UART_Debug_NUMBER_OF_DATA_BITS +
                        UART_Debug_NUMBER_OF_START_BIT) * UART_Debug_OVER_SAMPLE_COUNT;
        #else
    ...
    uint8 UART_Debug_ReadRxData(void) 
    {
        uint8 rxData;
            ...
            /* Disable Rx interrupt. */
                ...
                UART_Debug_DisableRxInt();
            #endif /* UART_Debug_RX_INTERRUPT_ENABLED */
            loc_rxBufferRead = UART_Debug_rxBufferRead;
            loc_rxBufferWrite = UART_Debug_rxBufferWrite;

            if( (UART_Debug_rxBufferLoopDetect != 0u) || (loc_rxBufferRead != loc_rxBufferWrite) )
            {
                rxData = UART_Debug_rxBuffer[loc_rxBufferRead];
                loc_rxBufferRead++;

                if(loc_rxBufferRead >= UART_Debug_RXBUFFERSIZE)
                {
                    loc_rxBufferRead = 0u;
                }
                /* Update the real pointer */
                UART_Debug_rxBufferRead = loc_rxBufferRead;
    ...
    void UART_Debug_PutChar(uint8 txDataByte) 
    {
                ...
                */
                ...
                do{
                    ...
                    #if ((UART_Debug_TXBUFFERSIZE > UART_Debug_MAX_BYTE_VALUE) && (CY_PSOC3))
                        /* Disable TX interrupt to protect variables that could change on interrupt */
                        CyIntDisable(UART_Debug_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                    loc_txBufferWrite = UART_Debug_txBufferWrite;
                    loc_txBufferRead = UART_Debug_txBufferRead;
                    #if ((UART_Debug_TXBUFFERSIZE > UART_Debug_MAX_BYTE_VALUE) && (CY_PSOC3))
                ...
                {
                    if(loc_txBufferWrite >= UART_Debug_TXBUFFERSIZE)
                    {
                        loc_txBufferWrite = 0u;
                    }
                    /* Add to the software buffer. */
                    UART_Debug_txBuffer[loc_txBufferWrite] = txDataByte;
                    loc_txBufferWrite++;
                    ...
                    /* Finally, update the real output pointer */
                        ...
                        CyIntDisable(UART_Debug_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                    UART_Debug_txBufferWrite = loc_txBufferWrite;
                    #if ((UART_Debug_TXBUFFERSIZE > UART_Debug_MAX_BYTE_VALUE) && (CY_PSOC3))
                        CyIntEnable(UART_Debug_TX_VECT_NUM);
                    #endif /* End TXBUFFERSIZE > 255 */
                }
    ...
    *******************************************************************************/
    void UART_Debug_PutString(const char8 string[]) 
    {
        ...
        if(UART_Debug_initVar != 0u)
        {
            ...
            {
                UART_Debug_PutChar((uint8)string[buf_index]);
                buf_index++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if(UART_Debug_initVar != 0u)
        {
            do
            {
                UART_Debug_PutChar(string[buf_index]);
                buf_index++;
