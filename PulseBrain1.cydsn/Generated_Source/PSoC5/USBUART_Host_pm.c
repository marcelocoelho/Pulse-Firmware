...
static USBUART_Host_BACKUP_STRUCT  USBUART_Host_backup;
...
extern void USBUART_Host_ConfigReg(void) ;
#if(USBUART_Host_EP_MM != USBUART_Host__EP_MANUAL)
    extern void USBUART_Host_Stop_DMA(uint8 epNumber) ;
#endif   /* End USBUART_Host_EP_MM != USBUART_Host__EP_MANUAL */

#if((USBUART_Host_DP_ISR_REMOVE == 0u) && (!CY_PSOC5A))


    /*******************************************************************************
    * Function Name: USBUART_Host_DP_Interrupt
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    CY_ISR(USBUART_Host_DP_ISR)
    {
        ...
        CY_GET_REG8(USBUART_Host_DP_INTSTAT_PTR);
    }

#endif /* (USBUART_Host_DP_ISR_REMOVE == 0u) && (!CY_PSOC5A) */


/*******************************************************************************
* Function Name: USBUART_Host_SaveConfig
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void USBUART_Host_SaveConfig(void) 
{
...
/*******************************************************************************
* Function Name: USBUART_Host_RestoreConfig
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void USBUART_Host_RestoreConfig(void) 
{
    if(USBUART_Host_configuration != 0)
    {
...
/*******************************************************************************
* Function Name: USBUART_Host_Suspend
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  USBUART_Host_backup.enable:  modified.
*
* Reentrant:
*  No.
*
...
void USBUART_Host_Suspend(void) 
{
    ...
    if((CY_GET_REG8(USBUART_Host_CR0_PTR) & USBUART_Host_CR0_ENABLE) != 0u)
    {   /* USB block is enabled */
        USBUART_Host_backup.enableState = 1u;
        ...
        /* Power Down Sequencing for USBIO for TO4*/
            ...
            USBUART_Host_PM_USB_CR0_REG &= \
                                    ~(USBUART_Host_PM_USB_CR0_PD_N | USBUART_Host_PM_USB_CR0_PD_PULLUP_N);
         #endif /* End !CY_PSOC5A */
        ...
        USBUART_Host_CR0_REG &= ~USBUART_Host_CR0_ENABLE;
            ...
            USBUART_Host_backup.mode = USBUART_Host_CR1_REG & USBUART_Host_CR1_REG_ENABLE;
            USBUART_Host_CR1_REG &= ~USBUART_Host_CR1_REG_ENABLE;
            ...
            USBUART_Host_USBIO_CR1_REG |= USBUART_Host_USBIO_CR1_IOMODE;
    ...
    {
        USBUART_Host_backup.enableState = 0u;
    }
    CyExitCriticalSection(enableInterrupts);

    /* Set the DP Interrupt for wake-up from sleep mode. */
    #if((USBUART_Host_DP_ISR_REMOVE == 0u) && (!CY_PSOC5A))
        CyIntSetVector(USBUART_Host_DP_INTC_VECT_NUM,   USBUART_Host_DP_ISR);
        CyIntSetPriority(USBUART_Host_DP_INTC_VECT_NUM, USBUART_Host_DP_INTC_PRIOR);
        CyIntClearPending(USBUART_Host_DP_INTC_VECT_NUM);
        CyIntEnable(USBUART_Host_DP_INTC_VECT_NUM);
    #endif /* (USBUART_Host_DP_ISR_REMOVE == 0u) && (!CY_PSOC5A) */
...
/*******************************************************************************
* Function Name: USBUART_Host_Resume
********************************************************************************
*
* Summary:
...
void USBUART_Host_Resume(void) 
{
    ...
    if(USBUART_Host_backup.enableState != 0u)
    {
        #if((USBUART_Host_DP_ISR_REMOVE == 0u) && (!CY_PSOC5A))
            CyIntDisable(USBUART_Host_DP_INTC_VECT_NUM);
        #endif /* End USBUART_Host_DP_ISR_REMOVE */
        ...
        /* USBIO Wakeup Sequencing for TO4 */
            ...
            CyDelayUs(40);
            /* Return VRegulator*/
            USBUART_Host_CR1_REG |= USBUART_Host_backup.mode;
            CyDelayUs(0);  /*~50ns delay */
            /* Enable USBIO */
...
/*******************************************************************************
* Function Name: USBUART_Host_Resume_Condition
********************************************************************************
* Summary:
...
uint8 USBUART_Host_Resume_Condition(void) 
{
    ...
    pinVal = USBUART_Host_USBIO_CR1_REG & USBUART_Host_USBIO_CR1_DP0;
