...
volatile uint8 USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_SIZE];
volatile uint8 USBUART_Host_lineChanged;
volatile uint16 USBUART_Host_lineControlBitmap;
volatile uint8 USBUART_Host_cdc_data_in_ep;
volatile uint8 USBUART_Host_cdc_data_out_ep;
...
/*******************************************************************************
* Function Name: USBUART_Host_DispatchCDCClassRqst
********************************************************************************
*
* Summary:
...
*
* Global variables:
*   USBUART_Host_lineCoding: Contains the current line coding structure.
...
*   USBUART_Host_lineControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   USBUART_Host_lineChanged: This variable is used as a flag for the
...
*
*******************************************************************************/
uint8 USBUART_Host_DispatchCDCClassRqst() 
{
    ...
    {   /* Control Read */
        ...
        {
            case USBUART_Host_CDC_GET_LINE_CODING:
                USBUART_Host_currentTD.count = USBUART_Host_LINE_CODING_SIZE;
                USBUART_Host_currentTD.pData = USBUART_Host_lineCoding;
    ...
    {   /* Control Write */
        switch (CY_GET_REG8(USBUART_Host_bRequest))
        {
            case USBUART_Host_CDC_SET_LINE_CODING:
                USBUART_Host_currentTD.count = USBUART_Host_LINE_CODING_SIZE;
                USBUART_Host_currentTD.pData = USBUART_Host_lineCoding;
                USBUART_Host_lineChanged |= USBUART_Host_LINE_CODING_CHANGED;
                requestHandled = USBUART_Host_InitControlWrite();
                break;

            case USBUART_Host_CDC_SET_CONTROL_LINE_STATE:
                USBUART_Host_lineControlBitmap = CY_GET_REG8(USBUART_Host_wValueLo);
                USBUART_Host_lineChanged |= USBUART_Host_LINE_CONTROL_CHANGED;
...
***************************************/
    ...
    /*******************************************************************************
    * Function Name: USBUART_Host_CDC_Init
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *   USBUART_Host_lineChanged: Initialized to zero.
    *   USBUART_Host_cdc_data_out_ep: Used as an OUT endpoint number.
    *
    * Reentrant:
    *  No.
    *
    *******************************************************************************/
    ...
    {
        USBUART_Host_lineChanged = 0u;
        USBUART_Host_EnableOutEP(USBUART_Host_cdc_data_out_ep);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_PutData
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *   USBUART_Host_cdc_data_in_ep: CDC IN endpoint number used for sending
    *     data.
    *
    * Reentrant:
    *  No.
    ...
    *******************************************************************************/
    void USBUART_Host_PutData(uint8* pData, uint16 length) 
    {
        ...
        if(length > USBUART_Host_EP[USBUART_Host_cdc_data_in_ep].bufferSize)
        {
            length = USBUART_Host_EP[USBUART_Host_cdc_data_in_ep].bufferSize;
        }
        USBUART_Host_LoadInEP(USBUART_Host_cdc_data_in_ep, pData, length);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_PutString
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void USBUART_Host_PutString(char8* string) 
    {
        ...
        {
            /* Limits length to maximim packet size for the EP */
            send_length = (str_length > USBUART_Host_EP[USBUART_Host_cdc_data_in_ep].bufferSize) ?
                          USBUART_Host_EP[USBUART_Host_cdc_data_in_ep].bufferSize : str_length;
             /* Enable IN transfer */
            USBUART_Host_LoadInEP(USBUART_Host_cdc_data_in_ep, (uint8 *)string, send_length);
            ...
            if(str_length > 0)
            {
                ...
                while(USBUART_Host_EP[USBUART_Host_cdc_data_in_ep].apiEpState ==
                                          USBUART_Host_IN_BUFFER_FULL);
            }
        }while(str_length > 0);
    }
    ...
    /*******************************************************************************
    * Function Name: USBUART_Host_PutChar
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        USBUART_Host_LoadInEP(USBUART_Host_cdc_data_in_ep, (uint8 *)&txDataByte, 1u);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_PutCRLF
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void USBUART_Host_PutCRLF(void) 
    {
        ...
        USBUART_Host_LoadInEP(USBUART_Host_cdc_data_in_ep, (uint8 *)txData, 2u);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_GetCount
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *   USBUART_Host_cdc_data_out_ep: CDC OUT endpoint number used.
    *
    *******************************************************************************/
    uint16 USBUART_Host_GetCount(void) 
    {
        ...
        if (USBUART_Host_EP[USBUART_Host_cdc_data_out_ep].apiEpState == USBUART_Host_OUT_BUFFER_FULL)
        {
            bytesCount = USBUART_Host_GetEPCount(USBUART_Host_cdc_data_out_ep);
        }
    ...
    /*******************************************************************************
    * Function Name: USBUART_Host_DataIsReady
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return(USBUART_Host_EP[USBUART_Host_cdc_data_out_ep].apiEpState);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_CDCIsReady
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *   USBUART_Host_cdc_data_in_ep: CDC IN endpoint number used.
    *
    *******************************************************************************/
    uint8 USBUART_Host_CDCIsReady(void) 
    {
        return(USBUART_Host_EP[USBUART_Host_cdc_data_in_ep].apiEpState);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_GetData
    ********************************************************************************
    *
    * Summary:
    *  Gets a specified number of bytes from the input buffer and places it in a
    *  data array specified by the passed pointer.
    *  USBUART_Host_DataIsReady() API should be called before, to be sure
    ...
    *******************************************************************************/
    ...
    {
        return(USBUART_Host_ReadOutEP(USBUART_Host_cdc_data_out_ep, pData, length));
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_GetAll
    ********************************************************************************
    *
    * Summary:
    *  Gets all bytes of received data from the input buffer and places it into a
    *  specified data array. USBUART_Host_DataIsReady() API should be called
    ...
    * Global variables:
    *   USBUART_Host_cdc_data_out_ep: CDC OUT endpoint number used.
    *   USBUART_Host_EP[].bufferSize: EP max packet size is used as a length
    ...
    *******************************************************************************/
    ...
    {
        return (USBUART_Host_ReadOutEP(USBUART_Host_cdc_data_out_ep, pData,
                                           USBUART_Host_EP[USBUART_Host_cdc_data_out_ep].bufferSize));
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_GetChar
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 USBUART_Host_GetChar(void) 
    {
        ...
        USBUART_Host_ReadOutEP(USBUART_Host_cdc_data_out_ep, &rxData, 1u);

        return(rxData);
    }

    /*******************************************************************************
    * Function Name: USBUART_Host_IsLineChanged
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *  USBUART_Host_transferState - it is checked to be sure then OUT data
    *    phase has been compleate, and data written to the lineCoding or Control
    *    Bitmap buffer.
    *  USBUART_Host_lineChanged: used as a flag to be aware that Host has been
    ...
    *******************************************************************************/
    uint8 USBUART_Host_IsLineChanged(void) 
    {
        ...
        if((USBUART_Host_lineChanged != 0u) &&
           (USBUART_Host_transferState == USBUART_Host_TRANS_STATE_IDLE))
        {
            state = USBUART_Host_lineChanged;
            USBUART_Host_lineChanged = 0u;
        }
        return(state);
    }
    ...
    /*******************************************************************************
    * Function Name: USBUART_Host_GetDTERate
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *  USBUART_Host_lineCoding: First four bytes converted to uint32
    ...
    *******************************************************************************/
    uint32 USBUART_Host_GetDTERate(void) 
    {
        ...
        /* Data terminal rate has little endian format. */
            ...
            ((uint8 *)&rate)[0u] = USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_RATE + 3u];
            ((uint8 *)&rate)[1u] = USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_RATE + 2u];
            ((uint8 *)&rate)[2u] = USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_RATE + 1u];
            ((uint8 *)&rate)[3u] = USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_RATE];
        #elif defined(__GNUC__) || defined(__ARMCC_VERSION)
            /* ARM compilers (GCC and RealView) are little-endian */
            USBUART_Host_CONVERT_DWORD *convert =
                (USBUART_Host_CONVERT_DWORD *)&USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_RATE];
            rate = convert->dword;
    ...
    /*******************************************************************************
    * Function Name: USBUART_Host_GetCharFormat
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *  USBUART_Host_lineCoding: used to get a parameter.
    *
    *******************************************************************************/
    uint8 USBUART_Host_GetCharFormat(void) 
    {
        return(USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_STOP_BITS]);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_GetParityType
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return(USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_PARITY]);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_GetDataBits
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return(USBUART_Host_lineCoding[USBUART_Host_LINE_CODING_DATA_BITS]);
    }


    /*******************************************************************************
    * Function Name: USBUART_Host_GetLineControl
    ********************************************************************************
    *
    * Summary:
    ...
    *
    * Global variables:
    *  USBUART_Host_lineControlBitmap: used to get a parameter.
    *
    *******************************************************************************/
    uint16 USBUART_Host_GetLineControl(void) 
    {
        return(USBUART_Host_lineControlBitmap);
    }
