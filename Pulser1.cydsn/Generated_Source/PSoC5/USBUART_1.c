...
extern volatile T_USBUART_1_EP_CTL_BLOCK USBUART_1_EP[];
    ...
    extern volatile uint8 USBUART_1_hidProtocol[];
...
void USBUART_1_Init(void) 
{
    ...
    /* ENABLING USBIO PADS IN USB MODE FROM I/O MODE */
        ...
        */
        USBUART_1_PM_USB_CR0_REG &= ~(USBUART_1_PM_USB_CR0_PD_N |USBUART_1_PM_USB_CR0_PD_PULLUP_N);
        ...
        USBUART_1_DM_INP_DIS_REG &= ~USBUART_1_DM_MASK;
        USBUART_1_DP_INP_DIS_REG &= ~USBUART_1_DP_MASK;
    ...
    CY_SET_REG8(&USBUART_1_ARB_RW1_WA_MSB_PTR[0u], 0u);
    ...
    /* Set the Data Endpoint 8 Interrupt. */
        ...
        CyIntSetVector(USBUART_1_ARB_VECT_NUM,   USBUART_1_ARB_ISR);
...
*
* Global variables:
*   USBUART_1_device: Contains the started device number from the
...
void USBUART_1_InitComponent(uint8 device, uint8 mode) 
{
    ...
    */
        ...
        for (interface = 0u; interface < USBUART_1_MAX_INTERFACES_NUMBER; interface++)
        {
            USBUART_1_hidProtocol[interface] = USBUART_1_PROTOCOL_REPORT;
        }
    #endif /* USBUART_1_ENABLE_HID_CLASS */
...
*
*******************************************************************************/
void USBUART_1_ReInitComponent() 
{
...
uint8 USBUART_1_CheckActivity(void) 
{
    ...
    CY_SET_REG8(USBUART_1_CR1_PTR, (r & ~USBUART_1_CR1_BUS_ACTIVITY));
...
uint16 USBUART_1_GetEPCount(uint8 epNumber) 
{
    ...
    if((epNumber > USBUART_1_EP0) && (epNumber < USBUART_1_MAX_EP))
    {
        ...
        count = ((uint16)((CY_GET_REG8(&USBUART_1_SIE_EP1_CNT0_PTR[ri]) & USBUART_1_EPX_CNT0_MASK) << 8u)
                         | CY_GET_REG8(&USBUART_1_SIE_EP1_CNT1_PTR[ri]))
    ...
    *     by the USBUART_1_LoadInEP() or USBUART_1_ReadOutEP() APIs.
    *
    * Parameters:
    ...
    *
    *******************************************************************************/
    void USBUART_1_InitEP_DMA(uint8 epNumber, uint8 *pData) 
    {
        uint16 src;
        uint16 dst;
    ...
    *******************************************************************************/
    void USBUART_1_Stop_DMA(uint8 epNumber) 
    {
        ...
        do
        {
            ...
            {
                CyDmaChDisable(USBUART_1_DmaChan[i]);
...
*
*******************************************************************************/
void USBUART_1_LoadInEP(uint8 epNumber, uint8 *pData, uint16 length)
                                                                        
{
    ...
    {
        ri = ((epNumber - USBUART_1_EP1) << USBUART_1_EPX_CNTX_ADDR_SHIFT);
        p = (uint8 *)&USBUART_1_ARB_RW1_DR_PTR[ri];
        ...
        CY_SET_REG8(&USBUART_1_SIE_EP1_CNT0_PTR[ri], (length >> 8u) | (USBUART_1_EP[epNumber].epToggle));
        CY_SET_REG8(&USBUART_1_SIE_EP1_CNT1_PTR[ri],  length & 0xFFu);
            ...
            USBUART_1_SIE_EP1_CR0_PTR[ri] = USBUART_1_EP[epNumber].epMode;
        #else
            ...
            if((pData != NULL) && (length > 0))
            {
                ...
                CyDmaChDisable(USBUART_1_DmaChan[epNumber]);
                CyDmaTdSetConfiguration(USBUART_1_DmaTd[epNumber], length, DMA_INVALID_TD,\
                                                                                    TD_TERMIN_EN | TD_INC_SRC_ADR);
                CyDmaTdSetAddress(USBUART_1_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
                /* Enable the DMA */
                CyDmaChSetInitialTd(USBUART_1_DmaChan[epNumber], USBUART_1_DmaTd[epNumber]);
                CyDmaChEnable(USBUART_1_DmaChan[epNumber], 1);
                /* Generate DMA request */
                USBUART_1_ARB_EP1_CFG_PTR[ri] |= USBUART_1_ARB_EPX_CFG_DMA_REQ;
                USBUART_1_ARB_EP1_CFG_PTR[ri] &= ~USBUART_1_ARB_EPX_CFG_DMA_REQ;
            ...
            if(pData != NULL)
            {
                ...
                CyDmaChDisable(USBUART_1_DmaChan[epNumber]);
                CyDmaTdSetConfiguration(USBUART_1_DmaTd[epNumber], length, USBUART_1_DmaTd[epNumber],\
                                                                                    TD_TERMIN_EN | TD_INC_SRC_ADR);
                CyDmaTdSetAddress(USBUART_1_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
                /* Clear Any potential pending DMA requests before starting the DMA channel to transfer data */
                CyDmaClearPendingDrq(USBUART_1_DmaChan[epNumber]);
                /* Enable the DMA */
                CyDmaChSetInitialTd(USBUART_1_DmaChan[epNumber], USBUART_1_DmaTd[epNumber]);
                CyDmaChEnable(USBUART_1_DmaChan[epNumber], 1);
            }
                                                                        ...
                                                                        
{
    ...
    #if(USBUART_1_EP_MM != USBUART_1__EP_DMAAUTO)
        uint16 xferCount;
    #endif /* End USBUART_1_EP_MM != $INSTANCE_NAME`__EP_DMAAUTO */
    ...
    if((epNumber > USBUART_1_EP0) && (epNumber < USBUART_1_MAX_EP) && (pData != NULL))
    {
        ...
        p = (uint8 *)&USBUART_1_ARB_RW1_DR_PTR[ri];
            ...
            CyDmaTdSetAddress(USBUART_1_DmaTd[epNumber],  LO16((uint32)p), LO16((uint32)pData));
            /* Enable the DMA */
            CyDmaChSetInitialTd(USBUART_1_DmaChan[epNumber], USBUART_1_DmaTd[epNumber]);
            CyDmaChEnable(USBUART_1_DmaChan[epNumber], 1);
...
void USBUART_1_EnableOutEP(uint8 epNumber) 
{
    ...
    if((epNumber > USBUART_1_EP0) && (epNumber < USBUART_1_MAX_EP))
    {
        ...
        USBUART_1_EP[epNumber].apiEpState = USBUART_1_NO_EVENT_PENDING;
        /* Write the Mode register */
        CY_SET_REG8(&USBUART_1_SIE_EP1_CR0_PTR[ri], USBUART_1_EP[epNumber].epMode);
    }
}
...
uint8 USBUART_1_GetEPAckState(uint8 epNumber) 
{
    ...
    {
        ri = ((epNumber - USBUART_1_EP1) << USBUART_1_EPX_CNTX_ADDR_SHIFT);
        cr = CY_GET_REG8(&USBUART_1_SIE_EP1_CR0_PTR[ri]) & USBUART_1_MODE_ACKD;
    }
...
void USBUART_1_SetPowerStatus(uint8 powerStatus) 
{
    ...
    *******************************************************************************/
    ...
    {
        return((CY_GET_REG8(USBUART_1_VBUS_PS_PTR) & USBUART_1_VBUS_MASK) ? 1u : 0u);
    }
