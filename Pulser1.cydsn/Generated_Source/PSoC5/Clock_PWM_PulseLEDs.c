...
void Clock_PWM_PulseLEDs_StopBlock(void) 
{
    if ((Clock_PWM_PulseLEDs_CLKEN & Clock_PWM_PulseLEDs_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_PWM_PulseLEDs_CLKEN_MASK;
                                ...
                                
{
    ...
    uint8 currSrc = Clock_PWM_PulseLEDs_GetSourceRegister();
    uint16 oldDivider = Clock_PWM_PulseLEDs_GetDividerRegister();
...
{
    return CY_GET_REG16(Clock_PWM_PulseLEDs_DIV_PTR);
}
...
{
    return Clock_PWM_PulseLEDs_MOD_SRC & Clock_PWM_PulseLEDs_SRC_SEL_MSK;
}
...
{
    Clock_PWM_PulseLEDs_PHASE = clkPhase & Clock_PWM_PulseLEDs_PHASE_MASK;
}
