...
/*******************************************************************************
* Function Name: Prism_LED_Clock_Start
********************************************************************************
*
* Summary:
...
void Prism_LED_Clock_Start(void) 
{
    ...
    Prism_LED_Clock_CLKEN |= Prism_LED_Clock_CLKEN_MASK;
	Prism_LED_Clock_CLKSTBY |= Prism_LED_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Prism_LED_Clock_Stop
********************************************************************************
*
* Summary:
...
void Prism_LED_Clock_Stop(void) 
{
    ...
    Prism_LED_Clock_CLKEN &= (uint8)(~Prism_LED_Clock_CLKEN_MASK);
	Prism_LED_Clock_CLKSTBY &= (uint8)(~Prism_LED_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Prism_LED_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Prism_LED_Clock_CLKEN & Prism_LED_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Prism_LED_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Prism_LED_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Prism_LED_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Prism_LED_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Prism_LED_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Prism_LED_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Prism_LED_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Prism_LED_Clock_GetSourceRegister();
    uint16 oldDivider = Prism_LED_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Prism_LED_Clock_CLKEN & Prism_LED_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Prism_LED_Clock_DIV_PTR, clkDivider);
                Prism_LED_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Prism_LED_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Prism_LED_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Prism_LED_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Prism_LED_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Prism_LED_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Prism_LED_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Prism_LED_Clock_MOD_SRC |= modeBitMask & (uint8)Prism_LED_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Prism_LED_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Prism_LED_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Prism_LED_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Prism_LED_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Prism_LED_Clock_MOD_SRC & (uint8)(Prism_LED_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Prism_LED_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Prism_LED_Clock_GetDividerRegister();
    uint8 oldSrc = Prism_LED_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Prism_LED_Clock_MOD_SRC |= CYCLK_SSS;
        Prism_LED_Clock_MOD_SRC =
            (Prism_LED_Clock_MOD_SRC & (uint8)(~Prism_LED_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Prism_LED_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Prism_LED_Clock_MOD_SRC & Prism_LED_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Prism_LED_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Prism_LED_Clock_PHASE = clkPhase & Prism_LED_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Prism_LED_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Prism_LED_Clock_PHASE & Prism_LED_Clock_PHASE_MASK;
}
