...
CY_ISR(USBUART_1_EP_0_ISR)
{
    ...
    if (bRegTemp & USBUART_1_MODE_ACKD)
    {
        modifyReg = 1u;
        if (bRegTemp & USBUART_1_MODE_SETUP_RCVD)
        {
            ...
            else
            {
                ...
                if(USBUART_1_ep0Mode & USBUART_1_MODE_SETUP_RCVD)
                {
        ...
        else if (bRegTemp & USBUART_1_MODE_IN_RCVD)
        {
            USBUART_1_HandleIN();
        }
        else if (bRegTemp & USBUART_1_MODE_OUT_RCVD)
        {
            USBUART_1_HandleOUT();
        }
        else
        ...
        if(modifyReg != 0u)
        {
            ...
            if((bRegTemp & USBUART_1_MODE_SETUP_RCVD) == 0)  /* Check if SETUP bit is not set, otherwise exit */
            {
                /* Update the count register */
                bRegTemp = USBUART_1_ep0Toggle | USBUART_1_ep0Count;
                CY_SET_REG8(USBUART_1_EP0_CNT_PTR, bRegTemp);
                if(bRegTemp == CY_GET_REG8(USBUART_1_EP0_CNT_PTR))   /* continue if writing was succesful */
                {
                    do
                    {
...
void USBUART_1_HandleSetup(void) 
{
    ...
    if(requestHandled & USBUART_1_MODE_SETUP_RCVD)
    {
...
*     current packet.
*  USBUART_1_lastPacketSize - remebers the USBFS_ep0Count value for the
*     next packet.
*  USBUART_1_transferByteCount - suma of the previos bytes transferred
...
void USBUART_1_LoadEP0(void) 
{
    ...
    while ((USBUART_1_currentTD.count > 0u) && (USBUART_1_ep0Count < 8u))
    {
        USBUART_1_EP0_DR0_PTR[USBUART_1_ep0Count] = *USBUART_1_currentTD.pData++;
        USBUART_1_ep0Count++;
        USBUART_1_currentTD.count--;
    }
    /* Support zero-length packet*/
    if( (USBUART_1_ep0Count > 0u) || (USBUART_1_lastPacketSize == 8u) )
    {
    ...
    USBUART_1_lastPacketSize = USBUART_1_ep0Count;
}
...
{
    uint16 xferCount;
    if(USBUART_1_currentTD.count == 0)
    {
        USBUART_1_InitZeroLengthControlTransfer();
    }
    else
    {
...
void USBUART_1_ControlWriteDataStage(void) 
{
    uint8 *pReg = (uint8 *)USBUART_1_EP0_DR0_PTR;

    USBUART_1_ep0Count = (CY_GET_REG8(USBUART_1_EP0_CNT_PTR ) & 0x0Fu) - 2u;

    USBUART_1_transferByteCount += USBUART_1_ep0Count;

    while ((USBUART_1_currentTD.count > 0u) && (USBUART_1_ep0Count > 0u))
    {
        *USBUART_1_currentTD.pData++ = CY_GET_REG8(pReg++);
        USBUART_1_ep0Count--;
        USBUART_1_currentTD.count--;
    }
    /* Update the data toggle */
